...
struct x264_t
{
    ...
    struct
    {
		...
		// 已确定帧类型，待编码帧，每一个GOP在编码前，每一帧的类型在编码前已经确定。
		// 当进行编码时，从这里取出一帧数据
        x264_frame_t **current;
        /* Unused frames: 0 = fenc, 1 = fdec */
		// 这个数组用于回收那些在编码中分配的frame空间，
		...
		/**
		 * 上一个IDR帧的帧序号，配合前面的i_frame，可以用来计算POC
		 */
		...
		/**
		 * 已经接收的输入帧数, frames结构体中i_input指示当前输入的帧的（播放顺序）序号。
		 */
	...
	// 指向当前重建帧，重建帧的帧号要比当前编码帧的帧号小1
    x264_frame_t    *fdec;

    /* references lists */
	// i_ref[0]和i_ref[1]分别表示前向和后向参考帧的数量
    int             i_ref[2];
	// fref[0]和fref[1]分别表示存放前向和后向参考帧的数组（参考帧均是重建帧）
